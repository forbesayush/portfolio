...
import { Globe2, ArrowRight, MapPin, Compass, ShieldCheck, CheckCircle2 } from 'lucide-react';
import { globalRoadmap, targetCountries } from '../data/portfolioData';
  ...
  return (
    <section id="global" className="py-24 relative bg-slate-50 dark:bg-obsidian-950 overflow-hidden transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        {/* Subtle Target Markets Exploration */}
        <div className="p-8 rounded-2xl bg-white dark:bg-obsidian-900/80 border border-slate-200 dark:border-white/10 shadow-card-light dark:shadow-2xl">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-6 border-b border-slate-200 dark:border-white/[0.08]">
            ...
            <span className="text-xs font-mono text-slate-700 dark:text-slate-400 bg-slate-100 dark:bg-obsidian-950 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-white/5 self-start md:self-auto font-medium">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {targetCountries.map((c) => (
              <div
                key={c.name}
                className="p-4 rounded-xl bg-slate-50 dark:bg-obsidian-850 border border-slate-200 dark:border-white/[0.06] hover:border-slate-300 dark:hover:border-white/20 hover:bg-white dark:hover:bg-obsidian-800 transition-all group shadow-2xs"
              >
                <div className="flex items-center justify-between mb-1.5">
                  <h4 className="text-base font-display font-bold text-slate-900 dark:text-white uppercase group-hover:text-accent dark:group-hover:text-accent-dark transition-colors">
                    {c.name}
                  </h4>
                  <MapPin className="w-3.5 h-3.5 text-slate-400 group-hover:text-accent dark:group-hover:text-accent-dark transition-colors" />
                </div>
                <div className="text-xs font-mono text-slate-700 dark:text-slate-300 mb-2 font-semibold">
                  {c.hub}
                </div>
