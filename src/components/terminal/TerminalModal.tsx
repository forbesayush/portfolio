...
export const TerminalModal: React.FC<TerminalModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm animate-in fade-in duration-200">
      <div
        className={`relative bg-[#1a1b26] border border-gray-300 rounded-2xl shadow-2xl overflow-hidden flex flex-col font-mono text-sm transition-all duration-300 ${
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-black/50 border-b border-white/10 select-none">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-xs text-slate-400 ml-2">ayush@portfolio-terminal:~</span>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            <button
              onClick={() => {
                soundManager.playClick();
                setLogs([{ id: '1', command: 'systemctl status portfolio.service', output: <div className="text-slate-300">portfolio.service: Ayush Chatterjee &bull; Active (running)</div> }]);
              }}
              onMouseEnter={() => soundManager.playHover()}
              className="min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white text-xs transition-colors"
        ...
        {/* Terminal Body */}
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 text-slate-100 font-mono text-xs overscroll-contain">
          {logs.map((item) => (
            <div key={item.id} className="space-y-1 text-left">
              ...
              <div className="pl-3 sm:pl-4 py-1 leading-relaxed">{item.output}</div>
            </div>
          ))}

          {/* Active Input Line: 16px font size on mobile to prevent iOS Safari auto-zoom */}
          <form
            onSubmit={(e) => {
              e.preventDefault();
              handleCommand(input);
          ...
          >
            <span className="text-cyber-cyan flex-shrink-0 text-xs sm:text-xs">root@ayushchatterjee:~$</span>
            <input
              ref={inputRef}
              type="text"
              value={input}
              ...
              className="flex-1 bg-transparent text-white font-mono focus:outline-none border-none p-0 text-base sm:text-xs min-h-[44px]"
              spellCheck={false}
              autoComplete="off"
            />
          </form>
        ...
        {/* Terminal Footer */}
        <div className="px-4 py-2.5 border-t border-white/5 bg-black/40 text-[11px] text-slate-400 flex flex-wrap justify-between font-mono gap-2 flex-shrink-0">
          <span>Type <span className="text-accent font-semibold">help</span> for commands</span>
          <span className="hidden sm:inline text-slate-500">Press ESC or click X to exit</span>
        </div>
      </div>
    </div>
  );
